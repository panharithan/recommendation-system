...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $insertQuery = $conn->prepare(
            "INSERT INTO recommendation_requests (user_id, email, token) VALUES (?, ?, ?)"
        );
        $insertQuery->bind_param("iss", $user["id"], $email, $token);

        if ($insertQuery->execute()) {
            // Call the function from send_email.php
            $feedback = sendRecommendationEmail(
                $email,
                $token,
                $user["username"],
                $user["email"]
...
$totalQuery = $conn->prepare(
    "SELECT COUNT(*) AS total FROM recommendation_requests WHERE user_id = ?"
);
...
$requestQuery = $conn->prepare(
    "SELECT id, email, status, created_at FROM recommendation_requests WHERE user_id = ? LIMIT ?, ?"
);
        ...
        <tr>
            ...
            <td>
                <?php if ($row["status"] === "pending"): ?>
                    <!-- Resend Button -->
                    <a href="resend_submission.php?id=<?= $row[
                        "id"
                    ] ?>" class="btn btn-warning btn-sm">Resend</a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endwhile; ?>
